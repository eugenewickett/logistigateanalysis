...
def plotLossVecs(lveclist, lvecnames=[], type='CI', CIalpha = 0.05,
    ...
    if type=='CI':
        ...
        for ind in range(numvecs):
            ...
            a0 = ax.annotate("|", xy=(lossint_hi[ind], lvecnames[ind]), **anno_args)
            a1 = ax.annotate("|", xy=(lossint_lo[ind], lvecnames[ind]), **anno_args)
        ...
        plt.xlabel('Loss', fontdict={'fontsize': 14, 'fontname': 'Trebuchet MS'})
...
def loss_squared(est, targ, paramDict={}):
    '''Squared loss function'''
    return np.linalg.norm(est-targ,2)

def loss_classification(est, targ, paramDict):
    '''Classification loss function with threshold t'''
